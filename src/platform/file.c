...
size_t read_file(FILE *fp, void *buf, size_t element_sz, size_t element_count, bool check_eof)
{
    ...
    LOG_CHECK(!check_eof || !eof, "read file failed with unexpected EOF")
    LOG_CHECK((total_read == element_count || eof || !err), "read file failed with code %d", err);
...
{
    LOG_ASSERT(fp, "fp is null");
    LOG_ASSERT(dest, "dest is null");
    LOG_ASSERT(dest_sz >= 1, "dest_sz is too small. Must be at least 1 byte");
    ...
    LOG_CHECK(!check_eof || !eof, "read file failed with unexpected EOF")
    LOG_CHECK((total_read == read_max || eof || !err), "read file failed with code %d", err);
