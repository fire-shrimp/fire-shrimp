...
bool open_directory(directory_stats *dir, string_buf *path)
{
    ...
    write_string_format(search, path->size + 2, path->size + 1, "%s\\*", path->buf);
    ...
    do {
        if (!(find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
            ...
            write_string_count(files[i].name, file_dest_sz, find_data.cFileName, file_count);
            ...
            while ((c = (*period++))) {
                if (c == '.') {
                    ...
                    write_string_count(files[i].extension, ext_dest_sz, period, ext_count);
                    break;
                }
            }
